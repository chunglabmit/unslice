...
pointFileExtensionToType = {"csv" : "CSV", "txt" : "CSV", "npy" : "NPY", "ims" : "Imaris"}
dataFileExtensions = ["tif", "tiff", "mhd", "raw", "ims", "nrrd","jp2"]
dataFileTypes = ["FileList", "TIF", "RAW", "NRRD", "Imaris","JP2"]
dataFileExtensionToType = { "tif" : "TIF", "tiff" : "TIF", "raw" : "RAW", "mhd" : "RAW", "nrrd": "NRRD", "ims" : "Imaris", "jp2":"JP2"}
...
def isFileExpression(source):
    ...
    if isFile(source):
        ...
        if m is not None:
            return True
        searchRegex = re.compile('.*\{(?P<digit>)\d\}.*').search
        m = searchRegex(source)
        if m is not None:
            return True
        else:
...
def dataFileNameToType(filename):
    ...
    if isFileExpression(filename):
        ...
        fext = fileExtension(filename)
       # print(fext, dataFileExtensions, dataFileExtensionToType)
        if fext in dataFileExtensions:
...
def dataFileNameToModule(filename):
    ...
    return importlib.import_module("hubris.IO." + ft)
...
def dataToRange(data, x = all, y = all, z = all, **args):
    ...
    if d == 1:
        return data[rr[0][0] : rr[0][1]].copy()
    elif d == 2:
        return data[rr[0][0] : rr[0][1], rr[1][0] : rr[1][1]].copy()
    elif d == 3:
        return data[rr[0][0] : rr[0][1], rr[1][0] : rr[1][1], rr[2][0] : rr[2][1]].copy()
    elif d == 4:
        return data[rr[0][0] : rr[0][1], rr[1][0] : rr[1][1], rr[2][0] : rr[2][1], :].copy()
    else:
...
def readData(source, **args):
    ...
    if source is None:
        ...
        return dataToRange(source.copy(), **args)
    else:
        raise RuntimeError('readData: cannot infer format of the requested data/file.')

        
def fastReadTiff(source):
    files = glob.glob(source)
    sample = cv2.imread(files[0],-1)
    outputs = numpy.zeros((len(files),sample.shape[0],sample.shape[1]), dtype='uint16')
    #print("Reading Files: ", flush=True)
    for z in tqdm(range(len(files))):
        outputs[z,:,:] = cv2.imread(files[z],-1)
    return outputs
